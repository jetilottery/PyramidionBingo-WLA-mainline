define([
    ...
    'game/bkSplashUtil'
], function (msgBus, SKBeInstant, config, resLib, splashUtil, PIXI, pixiResourceLoader, bkSplashUtil) {
    ...
    function onWindowResized() {
        ...
        for (let i = 0; i < gce.children.length; i++) {
            if (gce.children[i].id === 'changeBK') {
                setBgImageFromResLib(gce.children[i], orientation + 'Loading');
                gce.children[i].width = gameWidth + 'px';
                gce.children[i].height = gameHeight + 'px';
                gce.children[i].style.width = gameWidth + 'px';
                gce.children[i].style.height = gameHeight + 'px';
                break;
            }
        }

        bkSplashUtil.fitSize(gameWidth, gameHeight);
...
//        applyBrandCopyrightStyle(pdd);
        ...
        for (let j = 0; j < gce.children.length; j++) {
            if (gce.children[j].id === 'loadDiv') {
                gce.children[j].width = gameWidth + 'px';
                gce.children[j].height = gameHeight + 'px';
                gce.children[j].style.width = gameWidth + 'px';
                gce.children[j].style.height = gameHeight + 'px';
                break;
            }
        }
        //updateCanvasSize(gameWidth, gameHeight);
...
//    }
    ...
    function onSplashLoadDone() {
        ...
        for (var i = 0; i < gce.children.length; i++) {
            if (gce.children[i].id === 'changeBK') {
                setBgImageFromResLib(gce.children[i], orientation + 'Loading');
                break;
            }
...
//        }
    ...
    function initUI() {
        ...
        var pdd = config.predefinedStyle[orientation];
        bkSplashUtil.addCanvas(gameWidth, gameHeight, gce);
