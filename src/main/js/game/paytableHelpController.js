        ...
        if (SKBeInstant.isWLA()) {
            ...
            for (i = 0; i < revealConfigurations.length; i++) {
                ...
                for (j = 0; j < prizeTable.length; j++) {
                    tBody += '<tr><td nowrap="nowrap">' + (j + 1) + '</td><td nowrap="nowrap">' + prizeTable[j].description + '</td><td nowrap="nowrap">' + SKBeInstant.formatCurrency(prizeTable[j].prize).formattedAmount + '</td></tr>';
                }
                tBody += '</tbody></table>';
            }
        }
